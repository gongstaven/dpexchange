...
public class DpExchange extends JFrame implements ActionListener {
	...
	public DpExchange() {
		...
		jlabel_scale = new JLabel("<html><center>比例<br>" + scale
				+ "</center></html>");
	...
	public void actionPerformed(ActionEvent e) {
		...
		case 3:
		case 4:
			double sss = (Integer.valueOf(jcb_2.getSelectedItem().toString()) + 0.0)
					/ Integer.valueOf(jcb_1.getSelectedItem().toString());
			/** 保留三位小数点，四舍五入 */
			BigDecimal bigDecimal = new BigDecimal(sss);
			sss = bigDecimal.setScale(3, BigDecimal.ROUND_HALF_UP)
					.doubleValue();
			scale = sss;
			jlabel_scale.setText("<html><center>比例<br>" + scale
					+ "</center></html>");
			break;
		}
	}
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.contains("<dimen")) {
					...
					if (line.contains(jcb_1.getSelectedItem().toString())) {
						tmpStr = line
								.replace(jcb_1.getSelectedItem().toString(),
